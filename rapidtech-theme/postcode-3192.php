<?php
/*
Template Name: Postcode: Cheltenham (3192)
*/
require_once get_template_directory() . '/inc/location-template.php';

render_location_page([
    'suburb_name' => 'Cheltenham',
    'postcode' => '3192',
    'nearby_suburbs' => ['Highett', 'Moorabbin', 'Mentone', 'Beaumaris', 'Black Rock'],
    'distance_from_base' => '18',
    'landmarks' => ['Southland Shopping Centre', 'DFO Moorabbin', 'Sir William Fry Reserve', 'Cheltenham Golf Club', 'Kingston Heath Golf Club'],
    'geo_lat' => '-37.9548',
    'geo_long' => '145.0542',
    'population' => '21,500',
    'common_issues' => ['retail IT systems', 'golf club technology', 'professional services networks', 'home automation'],
    'suburb_description' => 'Reliable IT services for Cheltenham\'s sophisticated community. Located near Southland and the prestigious golf courses, we provide expert technology support for homes and businesses.',
    'unique_content' => '
        <h2>Premium IT Services for Cheltenham</h2>
        <p>Cheltenham occupies a prime position in Melbourne\'s Bayside region, known for world-class golf courses, excellent shopping at nearby Southland, and a mix of established homes and modern developments. The suburb attracts professionals who expect their technology to work flawlessly, and that\'s exactly what we deliver.</p>

        <h3>Business IT for Cheltenham\'s Commercial Sector</h3>
        <p>The area around Nepean Highway and Bay Road hosts numerous professional services firms, including accountants, lawyers, medical practices, and real estate agencies. These businesses require secure, compliant IT systems that protect client data while enabling efficient operations. We\'ve implemented secure document management systems, configured compliant backup solutions, and provided ongoing IT support that keeps these businesses operating smoothly.</p>

        <h3>Retail Technology Near Southland</h3>
        <p>With Southland Shopping Centre nearby, Cheltenham\'s retail sector thrives. Local businesses need reliable point-of-sale systems, inventory management, and secure payment processing. We support retailers from boutiques to service providers, ensuring their technology enhances rather than hinders customer experience. Our responsive support means technical issues don\'t translate to lost sales.</p>

        <h3>Golf Club and Sports Facility IT</h3>
        <p>Cheltenham is home to prestigious golf clubs including Kingston Heath and Cheltenham Golf Club. These facilities require specialised IT support—from member management systems to booking platforms and course monitoring technology. We\'ve worked with local clubs to modernize their IT infrastructure while respecting their traditions and member expectations.</p>

        <h3>Home Technology for Cheltenham Residents</h3>
        <p>Cheltenham\'s tree-lined streets house families who value quality and reliability. We provide home IT services that match these standards—from optimising Wi-Fi coverage in heritage homes with thick walls to setting up sophisticated home security systems. Our technicians understand that Cheltenham residents expect discreet, professional service that respects their homes and privacy.</p>

        <h3>Healthcare IT Compliance</h3>
        <p>With numerous medical practices in Cheltenham, healthcare IT compliance is crucial. We ensure patient data is secure, systems are backed up appropriately, and practitioners can access information when needed. Our understanding of healthcare privacy requirements makes us the trusted IT partner for Cheltenham\'s medical community.</p>
    '
]);
?>
